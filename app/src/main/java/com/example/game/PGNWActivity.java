...
public class PGNWActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pgnw);
        ...
        btnTimeTrial.setOnClickListener(view -> {
            ...
            startActivity(new Intent(PGNWActivity.this, PGNW_TT_ACTIVITY.class));
        });

        btnSurvivalMode.setOnClickListener(view -> {
            MainActivity.stopMusic();
            startActivity(new Intent(PGNWActivity.this, PGNW_SM_ACTIVITY.class));
        });

        btnFastMatch.setOnClickListener(view -> {
            MainActivity.stopMusic();
            startActivity(new Intent(PGNWActivity.this, PGNW_FM_ACTIVITY.class));
        });
        // Back button click listener
        btnBack.setOnClickListener(view -> {
            Intent intent = new Intent(PGNWActivity.this, CategoryActivity.class);
    ...
    protected void onResume() {
        super.onResume();
        MainActivity.resumeMusic(); // Resume music when returning from quiz
    }

    private void updateProgress() {
        // Fetch progress for all game modes
        int ttProgress = getProgressFromDB(dbHelper.getPGNWTTProgress(userId));
        int smProgress = getProgressFromDB(dbHelper.getPGNWSMProgress(userId));
        int fmProgress = getProgressFromDB(dbHelper.getPGNWFMProgress(userId));
